...
def main():
    ...
    labels = mRNA_data.loc["OSEVENT",samples].values.astype(np.int)
    ...
    print(">>> Select Gene")
    ttest_list, select_gene = first_select_gene(sam2num,good_sample,poor_sample,mRNA_data,geneList,gene2num,edge,args.SelectRatio)
    ...
    for i, sam in enumerate(samples):
        ...
        adjMat = construct_adjMat(new_edgeList,ttest_list[i],new_ngenes,gene2num,new_gene2num)
    ...
    for i, sam in enumerate(samples):
        ...
        cbow_data = make_cbow_data(path_data,sort_gene2num,args.WindowSize)
        vector = CBOW(cbow_data,args.VectorSize,select_gene)#count_genes
...
def first_select_gene(sample2num,good,poor,data,gene,gene2num,edgeList,select_ratio):
    all_sample = list(sample2num.keys())
    n_genes = len(gene)
...
def sampleClustering(data,genes,sampleList):
    ...
    train_expr = data.loc[genes,sampleList].values.astype(np.float32)
    expr_zscored = np.zeros(train_expr.shape)
...
def load_mRNA(expr_file,network_file) :
    ...
    tmp_expr = tmp_df.loc[features[1:], tmp_df.columns].values.astype(np.float32)

    patientList = tmp_df.columns
    osevent = tmp_df.loc["OSEVENT", patientList].values.astype(np.int)
